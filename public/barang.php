...
$produk_query = $db->query("SELECT p.id_produk, p.nama_produk, k.nama_kategori, p.harga, p.stok 
                            FROM produk p 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_produk) || empty($id_kategori) || empty($harga) || empty($stok)) {
        die('Semua field harus diisi.');
    }

    if ($id_produk) {
        // Edit Produk
        $stmt = $db->prepare("UPDATE produk SET nama_produk = ?, id_kategori = ?, harga = ?, stok = ? WHERE id_produk = ?");
        $stmt->execute([$nama_produk, $id_kategori, $harga, $stok, $id_produk]);
    } else {
        // Tambah Produk
        $stmt = $db->prepare("INSERT INTO produk (nama_produk, id_kategori, harga, stok) VALUES (?, ?, ?, ?)");
...
<body>
    <div class="w-64 h-screen bg-gray-800 fixed">
        ...
        <nav class="mt-4">
            ...
            <a href="products.php" class="block text-gray-300 py-4 px-6 hover:bg-gray-700 text-lg <?php echo ($current_page == 'barang.php') ? 'bg-gray-700' : ''; ?>">
                <i class="fas fa-box mr-2"></i>Products
            </a>
            <?php if ($user_role === 'admin') : // Menampilkan menu hanya untuk admin 
            ?>
